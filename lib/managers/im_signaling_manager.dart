...
class ImSignalingManager {
  ...
  /// [inviteID] 邀请ID
  /// [data] 自定义数据
  Future<String> cancel({@required String inviteID, @required String data}) => TencentImPlugin.cancel(inviteID: inviteID, data: data);

  /// 接收方接收邀请
  /// [inviteID] 邀请ID
  /// [data] 自定义数据
  Future<String> accept({@required String inviteID, @required String data}) => TencentImPlugin.accept(inviteID: inviteID, data: data);

  /// 接收方拒绝邀请
  /// [inviteID] 邀请ID
  /// [data] 自定义数据
  Future<String> reject({@required String inviteID, @required String data}) => TencentImPlugin.reject(inviteID: inviteID, data: data);

  /// 获取信令信息
  /// [msgID] 消息ID
  /// [Return] V2TIMSignalingInfo 信令信息，如果为 null，则 msg 不是一条信令消息。
  Future<SignalingInfoEntity> getSignalingInfo({@required String msgID}) =>
      TencentImPlugin.getSignalingInfo(message: FindMessageEntity(msgId: msgID));

  /// 添加邀请信令（可以用于群离线推送消息触发的邀请信令）
  /// [info] 信令信息对象
  Future<void> addInvitedSignaling({@required SignalingInfoEntity info}) => TencentImPlugin.addInvitedSignaling(info: info);
}
